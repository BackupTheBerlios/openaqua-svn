...
public class Main {
	...
	private static void execution () throws IOException  {		
		...
		//setup all threads
		HibernateGenerator[] threadArray = new HibernateGenerator[Configuration.getInstance().getMaxConnections()];
		int readInstance = 0;
		int writeInstance = 0;
		for (int i = 0; i < Configuration.getInstance().getMaxConnections(); i++) {
			if (i%2==1) {		threadArray[i] = new HibernateGenerator( "" + i, serverKey, true);
								readInstance++;
			} else {			threadArray[i] = new HibernateGenerator( "" + i, serverKey, false);
								writeInstance++;
			}
		}
	...
	private static boolean setupDatabase(){
		...
		try {
			...
			HibernateGenerator main = new HibernateGenerator("main", serverKey, false);
			main.setupDatabase();
