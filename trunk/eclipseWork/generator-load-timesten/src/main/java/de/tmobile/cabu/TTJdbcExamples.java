 ...
 * Driver.
 *
 * (c) Copyright 1996-2004, TimesTen, Inc.
 * All rights reserved.
 *
 * Please consult the README files
 *      <install_dir>/jdbc/README.TXT
 * and
 *      <install_dir>/jdbc/demo/README.TXT
 * for instructions and prerequisites before running this demo.
 *
 ...
 *      java TTJdbcExamples RunData
 *      java TTJdbcExamples -run 2 RunData
 *      java TTJdbcExamples -client -run 2 -dsn RunDataCS
 *
 */
...
public class TTJdbcExamples
{
	...
	public int runExamples()
	{
		...
		try {
			...
			case 4:
				// Demonstrates how to checkpoint
				checkpoint();
				break;
	...
	//
	// Description:
	//    returns the number of checkpoints taken on the data store
	//
	// Assumptions:
	...
	//
	// Returns:
	//    number of checkpoints taken on the data store
	//
	// ******************************************************************
	private int numOfCheckpoints(Connection con) {
		int num = -1;
		Statement stmt = null;
		ResultSet rs = null;
		...
		try {
			...
			rs = stmt.executeQuery("SELECT DS_CHECKPOINTS FROM sys.monitor");
	...
	//
	// Description:
	//    shows how to call a checkpoint
	//
	// ******************************************************************
	...
	throws ClassNotFoundException, SQLException
	{
		...
		try {
			...
			System.out.println("Number of checkpoints: " +
					numOfCheckpoints(con));

			System.out.println("Call ttCkptBlocking.");
			cStmt = con.prepareCall("{CALL ttCkptBlocking()}");
			cStmt.execute();
			...
			System.out.println("Number of checkpoints before commit: " +
					numOfCheckpoints(con));
			con.commit();
			inXact = false;
			CheckIfStopIsRequested();

			System.out.println("Number of checkpoints after commit: " +
					numOfCheckpoints(con));
