// Simple_util.cpp,v 1.10 2004/05/02 05:09:42 ossama Exp
...
OaSimpleServer<Servant>::OaSimpleServer ( void )
      : ior_output_file_ ( 0 ),
      naming_ ( 0 ),
      ins_ ( 0 )
{
...
template <class Servant>
int OaSimpleServer<Servant>::parse_args ( void )
{
...
template <class Servant>
int
OaSimpleServer<Servant>::test_for_ins ( CORBA::String_var ior )
{
   CORBA::ORB_var orb = this->orb_manager_.orb ();

   if ( TAO_debug_level > 0 )
      ACE_DEBUG ( ( LM_DEBUG, "Adding (KEY:IOR) %s:%s\n", this->ins_, ior.in () ) );
...
template <class Servant>
int OaSimpleServer<Servant>::init ( const char *servant_name,
                                    int argc,
                                    char *argv[]
                                    ACE_ENV_ARG_DECL )
{
   ...
   if ( this->orb_manager_.init_child_poa ( argc, argv, "child_poa" ACE_ENV_ARG_PARAMETER ) == -1 )
   {
   ...
   ACE_TRY
   {
      CORBA::String_var str = this->orb_manager_.activate_under_child_poa ( servant_name, &this->servant_ ACE_ENV_ARG_PARAMETER );
      ACE_TRY_CHECK;
...
template <class Servant>
int OaSimpleServer<Servant>::run ( ACE_ENV_SINGLE_ARG_DECL )
{
   // Run the main event loop for the ORB.
   if ( this->orb_manager_.run ( ACE_ENV_SINGLE_ARG_PARAMETER ) == -1 )
   {
      ACE_ERROR_RETURN ( ( LM_ERROR, "OaSimpleServer_i::run" ), -1 );
   }
...
template <class Servant>
int OaSimpleServer<Servant>::register_name ( void )
{
   CORBA::ORB_var orb = this->orb_manager_.orb ();
   ...
   ACE_TRY
   {
      ...
      naming_client_->rebind ( bindName, object.in() ACE_ENV_ARG_PARAMETER );
      ACE_TRY_CHECK;
      ...
      if ( this->ins_ )
   {
      CORBA::String_var ior = orb->object_to_string ( object.in () );
         ...
         {
            ACE_ERROR_RETURN ( ( LM_ERROR, "test_for_ins (): failed\n" ), -1 );
         }
      }
   }
...
OaSimpleClient<INTERFACE_OBJECT, Var>::OaSimpleClient ( void )
      : ior_ ( 0 )
      ,      shutdown_ ( 0 )
{
   //no-op
...
template <class INTERFACE_OBJECT, class Var>
int OaSimpleClient<INTERFACE_OBJECT, Var>::read_ior ( char *filename )
{
...
template <class INTERFACE_OBJECT, class Var>
int
OaSimpleClient<INTERFACE_OBJECT, Var>::parse_args ( void )
{
...
template <class INTERFACE_OBJECT, class Var>
int OaSimpleClient<INTERFACE_OBJECT, Var>::init ( const char *name,
                                                  int argc,
                                                  char **argv )
{
   this->argc_ = argc;
...
template <class INTERFACE_OBJECT, class Var>
int
OaSimpleClient<INTERFACE_OBJECT, Var>::obtain_initial_references ( ACE_ENV_SINGLE_ARG_DECL_NOT_USED )
{
...
template <class INTERFACE_OBJECT, class Var>
int
OaSimpleClient<INTERFACE_OBJECT, Var>::shutdown ( void )
{
...
template <class INTERFACE_OBJECT, class Var>
void
OaSimpleClient<INTERFACE_OBJECT, Var>::shutdown ( int flag )
{
   // Fills the flag
   shutdown_ = flag;
}
