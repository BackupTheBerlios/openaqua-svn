...
void Workbench::slotFileOpen()
{
   ...
   //where to open
   CoaString openFilesPath = settings.value(LASTFILEPATH, ".").toString();

   //the dialog
   CoaStringList files = CoaFileDialog::getOpenFileNames(
      magicalParent(), tr("CoaFileDialog::getOpenFileNames()"),
      openFilesPath,
      tr("Coa Action Description (*.xml);;All Files (*)"));

   //the result
   for (int i = 0; i < files.count(); ++i){
      openFilesPath = files[0];
      INF(QString("Open Files in dir %1").arg(openFilesPath));
      MainWindow *window = new MainWindow("MainWindow", magicalParent(), 0);
      window->initialize(); //init the DataModel?
      bool ok = window->slotFileOpen(openFilesPath);
      if (ok == true){
         initWindow(window);
         addWindow(window);
         showWindow(window);
   ...
   settings.setValue(LASTFILEPATH, openFilesPath);
   settings.endGroup();
...
void Workbench::addDocument(ActionDocument *document)
{
   BEGIN;
   Q_UNUSED(document);
}
...
{
   BEGIN;
   MainWindow *window = new MainWindow("MainWindow", magicalParent());
   window->initialize(); //init the DataModel?
...
void Workbench::slotFileSaveAs()
{
   ...
   if (workspace != 0){
      ...
      if (widget != 0) {
         MainWindow *window = qobject_cast<MainWindow*>(widget);
         if (window != 0) window->slotFileSaveAs();
      }
