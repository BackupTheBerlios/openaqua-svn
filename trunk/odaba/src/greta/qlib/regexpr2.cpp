...
public:
    ...
    virtual bool peek_this( peek_param<typename sub_expr<IterT>::char_type> & peek ) const
    {
...
public:
    ...
    virtual bool peek_this( peek_param<typename match_wrapper<IterT, SubExprT>::char_type> & peek ) const
    {
...
public:
    ...
    virtual bool peek_this( peek_param<typename match_char<IterT>::char_type> & peek ) const
    {
        _do_peek_this( peek, m_ch );
        return true;
    }
private:
    static bool eq( typename sub_expr<IterT>::char_type left, typename sub_expr<IterT>::char_type right )
    {
        return sub_expr<IterT>::traits_type::eq( left, right );
    }
    static bool eq( typename sub_expr<IterT>::char_type left, char_nocase<typename sub_expr<IterT>::char_type> right )
    {
        return sub_expr<IterT>::traits_type::eq( left, right.m_chlo ) ||
               sub_expr<IterT>::traits_type::eq( left, right.m_chhi );
    }
    static void _do_peek_this( peek_param<typename match_char<IterT>::char_type> & peek, typename sub_expr<IterT>::char_type ch )
    {
        peek.m_cchars           = 1;
        peek.m_rgchars[0]       = ch;
        peek.m_must_have.m_has  = false;
    }
    static void _do_peek_this( peek_param<typename match_char<IterT>::char_type> & peek, char_nocase<typename sub_expr<IterT>::char_type> ch )
    {
        peek.m_cchars           = 2;
...
private:
    ...
    bool _do_match_this( match_param<IterT> & param, IterT & icur REGEX_VC6(COMMA CStringsT) ) const
    {
        ...
        {
            if( eos_t<CStringsT>::eval( param, icur_tmp ) || ! match_literal<IterT>::traits_type::eq( *ithis, *icur_tmp ) )
                return false;
        }
        icur = icur_tmp;
        return true;
...
public:
    typedef typename match_literal<IterT>::char_type       char_type;
    typedef typename match_literal<IterT>::traits_type     traits_type;
    typedef typename match_literal<IterT>::string_type     string_type;
    typedef typename match_literal<IterT>::iterator        iterator;
    typedef typename match_literal<IterT>::const_iterator  const_iterator;
    ...
    match_literal_nocase_t( iterator ibegin, const_iterator iend, regex_arena & arena )
        : match_literal<IterT>( ibegin, iend )
        , m_szlower( arena_allocator<char_type>( arena ).allocate( match_literal<IterT>::m_dist ) )
    {
...
{
    public:
	typedef typename match_group_base<IterT>::char_type char_type;
...
{
    typedef typename independent_group<IterT>::char_type char_type;
...
{
    typedef typename independent_group_base<IterT>::char_type char_type;
...
REGEXPR_H_INLINE void basic_rpattern_base<IterT, SyntaxT>::_common_init( REGEX_FLAGS flags )
{
    ...
    REGEX_ASSERT( 0 == this->m_pfirst );
    this->m_pfirst = pgroup;

    // Calculate the width of the pattern and all groups
    this->m_nwidth = pgroup->group_width( rggroups, this->m_invisible_groups );
    ...
    {
        this->m_flags = ( REGEX_FLAGS ) ( this->m_flags & ~RIGHTMOST );
        this->m_floop = false;
    }
    ...
    std::vector<detail::match_group_base<IterT>*> & rggroups )
{
    ...
    if( this->m_pat->end() != ipat && NO_TOKEN != ( tok = sy.ext_token( ipat, this->m_pat->end() ) ) )
    {
        ...
        if( END_GROUP != sy.reg_token( itemp = ipat, this->m_pat->end() ) )
        {
            ...
            case EXT_NOBACKREF:
                ...
                this->m_invisible_groups.push_back( this->m_cgroups );
    ...
    if( 0 != pgroup.get() )
    {
        ...
        if( detail::is_random_access<IterT>::value && must.m_has && 0 == pgroup->group_number() )
        {
            ...
            this->m_search = new( this->m_arena ) detail::boyer_moore<iter_type>
