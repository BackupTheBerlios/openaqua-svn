...
class backref_tag : public std::pair<IterT, IterT>
{
    ...
    {
        sout.write( std::pair<IterT, IterT>::first, static_cast<std::streamsize>( std::distance( std::pair<IterT, IterT>::first, std::pair<IterT, IterT>::second ) ) );
    }
...
public:
    ...
    IterT begin() const
    {
        return std::pair<IterT, IterT>::first;
    }

    IterT end() const
    {
        return std::pair<IterT, IterT>::second;
    }

    string_type const str() const
    {
        return matched ? string_type( std::pair<IterT, IterT>::first, std::pair<IterT, IterT>::second ) : string_type();
    }
...
public:
    ...
    ) const
    {
        ...
        if( len == impl::npos || pos + len >= str.size() )
            iend = IterT(str.end());
        else
