...
{
  ODABAClient      uti_client(argc > 1 ? argv[1] : NULL,"SetupDB",argv[0],APT_Console);
  logical          renamed = YES;
  logical          term = NO;
BEGINSEQ

  CSUtilityHandle  uti_handle(uti_client,"SetupDB",argc > 1 ? argv[1] : NULL,argv[0],APT_Console);
  ...
  if ( SetupDB(uti_handle,renamed) )               ERROR
