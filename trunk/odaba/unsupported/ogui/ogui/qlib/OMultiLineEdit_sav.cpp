...
\brief    RichTextEdit
          This  Control enables the user to edit  text with styles, eg fonts and
          color and alignment.
          ...
          you drop it
          from  any text/html capable source the  formated text will drop at the
          cursor  position.  if  you  had  any  <img>  tags  selected  in  your 
          dropsource  they  will  appear  with  the source dimensions but with a
          nasty placeholder image instead of the source.
          ...
          "/images/logos/run.png" will be found as d:\images\logosun.png
          "images/logos/run.png" will be found d:\dll\images\logosun.png

\date     $Date: 2006/04/03 10:51:26,18 $
...
logical OMultiLineEdit :: AddHighlightClass (char *names, DSyntaxClass &dsyntaxclass, DFont &dfont )
{
  ...
  DLL(char)   expressions((DLL(char)&)dsyntaxclass.get_expressions()); 
  while ( string = expressions.GetNext() )
...
BEGINSEQ
  ...
  scrollStore();
// disabled during debug of input help
//  if ( input_help && !input_help->HasFocus() )
//    input_help->Hide();
...
BEGINSEQ
  ...
  // keywords 
  highlighter->Class( "keyword", (QFont&)QFont( SZ2QS( "COURIER" ), 10 ), (QColor&)QColor( 0, 0, 255 ) );
  ...
  // odaba keywords // 
  highlighter->Class( "okeyword", (QFont&)QFont( SZ2QS( "COURIER" ), 10 ), (QColor&)QColor( 23, 164, 8 ) );
  ...
  // numbers //
  highlighter->Class( "number", (QFont&)QFont( SZ2QS( "COURIER" ), 10 ), (QColor&)QColor( 255, 0, 0 ) );
  highlighter->Add( "number", "\\b\\d+\\b" );
  // strings //
  highlighter->Class( "string", (QFont&)QFont( SZ2QS( "COURIER" ), 10), (QColor&)QColor( 0, 80, 0) );
...
//  highlighter->Add( "string", "\\\"", "([^(\\\\){1}]|(\\\\){2}|[^(\\\\){3}]|(\\\\){4})\\\"" );
  ...
  // operators //
  highlighter->Class( "operator", (QFont&)QFont( SZ2QS( "COURIER" ), 10 ), (QColor&)QColor( 0, 0, 180 ) );
  ...
  highlighter->Add( "operator", "\\^" );
  // preprocessor //
  highlighter->Class( "preprocessor", (QFont&)QFont( SZ2QS( "COURIER" ), 10 ), (QColor&)QColor( 80, 80, 255) );
  highlighter->Add( "preprocessor", "#.+\\b" );
  // comments //
  highlighter->Class( "comment", (QFont&)QFont( SZ2QS( "COURIER" ), 10 ), (QColor&)QColor( 80, 80, 80 ) );
...
logical OMultiLineEdit :: Initialize ( )
{
  ...
  //setTabChangesFocus(true); // tab and shift tab are handled separate
...
BEGINSEQ
  if ( !context_menu )
  {
    ...
    def->setTitle(SZ2QS("Default"));
    context_menu->addMenu(def);
  ...
  oeh()->SetActionDisabled("Import" ,NULL,IsReadOnly());
...
logical OMultiLineEdit :: SetDefaultReactions ( )
{
  ...
  SetGUIDefReaction("Import"      ,OEHShortcut("CTRL+I"),"___ContextMenu")->SetSeparated(YES);
    ...
    else
    {
      ...
      if ( rxWS.indexIn(qsLine,0) != -1 )
      {
        ...
        if ( iLen > iTabWidth*abs(iDirection) )
          SetTextSelection(DPoint(1,iLine),DPoint(iTabWidth*abs(iDirection)+1,iLine),1);
        else
          SetTextSelection(DPoint(1,iLine),DPoint(iLen+1,iLine),1);
...
void OMultiLineEdit :: SetTextPosition (DPoint dposition )
{
  ...
  setFocus();
  if( (cursor = textCursor()) == QTextCursor())
...
void OMultiLineEdit :: paintEvent (QPaintEvent *pQPaintEvent )
{
 ...
 QRect  r,rr;
  if(bHighLightParagraphBackground && hasFocus()){
    QPainter p(viewport());
    r = LineRect();
    rr = r; // real rect is without scrolling
    rr.setY(rr.y());
    p.setClipping(false);
    p.setPen(Qt::NoPen);
    p.setBrush(bc);
