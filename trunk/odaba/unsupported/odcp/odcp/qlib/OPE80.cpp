    ...
    *cmdlptr = 0;
    ...
    if ( *section )
    {
      ...
      if ( *opath && opath[strlen(opath)-1] != '\\' && opath[strlen(opath)-1] != '/' )
        strcat(opath,"/");
      if ( !LoadLibrary(strcat(strcpy(dllpath,opath),"SOS80.DLL")) )
        LoadLibrary("SOS80.DLL");
      if ( !LoadLibrary(strcat(strcpy(dllpath,opath),"OPA80.DLL")) )
        LoadLibrary("OPA80.DLL");
      if ( !LoadLibrary(strcat(strcpy(dllpath,opath),"OGUI80.DLL")) )
        LoadLibrary("OGUI80.DLL");        
      if ( !(lhinst = LoadLibrary(strcat(strcpy(dllpath,opath),"OADI80.DLL"))) )
