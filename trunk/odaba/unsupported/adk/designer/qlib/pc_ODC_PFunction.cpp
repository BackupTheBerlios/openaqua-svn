...
\date     $Date: 2006/04/24 16:12:42,49 $
...
MethodTypes pc_ODC_PFunction :: GetImplementationType (CTX_GUIBase *ctxptr, logical setup_opt )
{
...
logical pc_ODC_PFunction :: InitializePFunction (PropertyHandle *impcls_ph, pODE *ode_ctx )
{
...
BEGINSEQ
  ...
  if ( pc_ODC_ImpClass(impcls_ph).InitFunction() )   ERROR
...
logical pc_ODC_PFunction :: InitializePFunction (PropertyHandle *func_ph )
{
  logical         term         = NO;
BEGINSEQ
...
//  *GPH("__GUID")   = "";
  ...
  *GPH("export")     = NO;
  ...
  pc_ODC_FunctImp(phimplementations).
...
logical pc_ODC_PFunction :: IsNewInstance ( )
{
...
logical pc_ODC_PFunction :: ProvideConstructor ( )
{
...
logical pc_ODC_PFunction :: ProvideDestructor (logical virtual_opt )
{
  PropertyHandle *clsnames = GetParentProperty()->GPH("sys_ident");
  char            idkey[ID_SIZE];
  logical          term     = NO;
...
char pc_ODC_PFunction :: ProvideImplementation ( )
{
  PropertyHandle     *impls   = GPH("implementations");
  logical             created = NO;
BEGINSEQ
